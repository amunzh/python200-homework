...
tool = [
    {
        ...
        'function': {
            ...
            'description': 'Take Celsius temperature converts it to Fahrenheit and return redult as a formatted string.',
            'parameters': {
                'type': 'object',
                'properties': {
                    "celsius": {"type": "number"}},
...
print(f'{celsius_to_fahrenheit(0)}\n{celsius_to_fahrenheit(100)}\n{celsius_to_fahrenheit(-40)}')
...
# 1. Because the tools doesn't have function to convert Celsius to Fahrenheit it won't trigger a tool call
# 2. It will make only 1 API call because it will not use any extra tools
...
def run_agent(user_prompt: str) -> str:
    ...
    SYSTEM_PROMPT = '''You are a simple assistant that can tell the current time and convert Celsius temperature to Fahrenheit.
                     Use the tool get_current_time whenever a user asks about the time or tool celsius_to_fahrenheit whenever a user asks to convert temperature from Celsius to Fahrenheit.'''
...
# The tool was called because the question directly ask to convert celsius to ferenheit
response_b = run_agent("What is the boiling point of water in plain English?")
print("Response B:", response_b)
# Non tools was called because prompt asked for a general information and not to convert temperature, so agent didn't need to call any tools.
...
#Q6
import json
#System gives information what assistent do and how it should behave. User is a message/question from the user. Assistant is a response from the model. Tool is what kind of tool had been called and what paremetrs does it have.
...
def compute_correlation(col1: str, col2: str) -> dict:
    """
    Compute the Pearson correlation between two columns from the active CSV.
...
# Smolagents automatically generates tools dictionary, it needs a description, args, and returns to produce a good description
...
# 1. Both of them provided scatter plots. ToolCallAgent didn't produce plot with green dots and had names as stored while CodeAgent had creatted green dotted plot with proper names and plot ready to use.
# 2. ToolCallAgent calls already existed code(functions) and using it to answer user's prompt. It is better to use when compliting a specific task and you have a specific steps system have to follow
# CodeAgent is better for more open ended tasks where the code doesn't have to be strict and not be used a lot of times.

# Q9
# 1. If a company has a large set of documents that needs to be scanned and converted to a specific format on an everyday basis(maybe less), the ToolCallAgent would be best because of the specific steps the agent has to follow; the results for all these documents would be formatted the same, no matter how the user formats the question 
# 2. CodeAgent can generate Malicious code that can cause damage to already existing information, altering it or deleting it. Without control, the code could become overcomplicated and erroneous, which would be hard to fix.
